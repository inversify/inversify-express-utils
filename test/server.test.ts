...
describe("Integration Tests:", () => {
    ...
    describe("Routing & Request Handling:", () => {
        it("should work for async controller methods", (done) => {
            ...
            void supertest(server.build()).get("/").expect(200, "GET", done);
        });
        ...
        it("should work for async controller methods that fails", (done) => {
            ...
            server = new InversifyExpressServer(container);
            void supertest(server.build()).get("/").expect(500, done);
        });
        ...
        it("should work for each shortcut decorator", (done) => {
            ...
            class TestController {
                ...
                @httpPut("/")
                public putTest(req: Request, res: Response) {
                    res.send("PUT");
                }

                @httpPatch("/")
                public patchTest(req: Request, res: Response) {
                    res.send("PATCH");
                }

                @httpHead("/")
                public headTest(req: Request, res: Response) {
                    res.send("HEAD");
                }
            ...
            const patch = () => {
                void agent.patch("/").expect(200, "PATCH", head);
            };
            const put = () => {
                void agent.put("/").expect(200, "PUT", patch);
            };
            const post = () => {
                void agent.post("/").expect(200, "POST", put);
            };
            const get = () => {
                void agent.get("/").expect(200, "GET", post);
            };
        ...
        it("should work for more obscure HTTP methods using the httpMethod decorator", (done) => {
            ...
            server = new InversifyExpressServer(container);
            void supertest(server.build()).propfind("/").expect(200, "PROPFIND", done);
        });
        ...
        it("should use returned values as response", (done) => {
            ...
            server = new InversifyExpressServer(container);
            void supertest(server.build()).get("/").expect(200, JSON.stringify(result), done);
        });
        ...
        it("should use custom router passed from configuration", () => {
            ...
            const expectedNotFound1 = supertest(app).get("/casesensitive/endpoint").expect(404);

            const expectedNotFound2 = supertest(app).get("/CaseSensitive/endpoint").expect(404);

            return Promise.all([expectedSuccess, expectedNotFound1, expectedNotFound2]);
        });
        ...
        it("should use custom routing configuration", () => {
            ...
            return supertest(server.build()).get("/api/v1/ping/endpoint").expect(200, "pong");
        });
    ...
    describe("Middleware:", () => {
        ...
        it("should call method-level middleware correctly (POST)", (done) => {
            ...
            void agent.post("/").expect(200, "POST", () => {
                expect(spyA).toHaveBeenCalledTimes(1);
                expect(spyB).toHaveBeenCalledTimes(1);
                expect(spyC).toHaveBeenCalledTimes(1);
                expect(result).toBe("abc");
        ...
        it("should call method-level middleware correctly (PATCH)", (done) => {
            ...
            void agent.patch("/").expect(200, "PATCH", () => {
                expect(spyA).toHaveBeenCalledTimes(1);
                expect(spyB).toHaveBeenCalledTimes(1);
                expect(spyC).toHaveBeenCalledTimes(1);
                expect(result).toBe("abc");
        ...
        it("should call method-level middleware correctly (HEAD)", (done) => {
            ...
            void agent.head("/").expect(200, "HEAD", () => {
                expect(spyA).toHaveBeenCalledTimes(1);
                expect(spyB).toHaveBeenCalledTimes(1);
                expect(spyC).toHaveBeenCalledTimes(1);
                expect(result).toBe("abc");
    ...
    describe("Parameters:", () => {
        it("should bind a method parameter to the url parameter of the web request", (done) => {
            ...
            class TestController {
                @httpGet(":id")
                public getTest(@requestParam("id") id: string, req: Request, res: Response) {
                    return id;
                }
            }

            server = new InversifyExpressServer(container);
            void supertest(server.build()).get("/foo").expect(200, "foo", done);
        });
        ...
        it("should bind a method parameter to the request object", (done) => {
            ...
            server = new InversifyExpressServer(container);
            void supertest(server.build()).get("/GET").expect(200, "GET", done);
        });
        ...
        it("should bind a method parameter to the response object", (done) => {
            ...
            server = new InversifyExpressServer(container);
            void supertest(server.build()).get("/").expect(200, "foo", done);
        });
        ...
        it("should bind a method parameter to a query parameter", (done) => {
            ...
            server = new InversifyExpressServer(container);
            void supertest(server.build()).get("/").query("id=foo").expect(200, "foo", done);
        });
        ...
        it("should bind a method parameter to the request body", (done) => {
            ...
            void supertest(server.build()).post("/").send(body).expect(200, body, done);
        });
        ...
        it("should bind a method parameter to the request headers", (done) => {
            ...
            class TestController {
                @httpGet("/")
                public getTest(@requestHeaders("testhead") headers: Record<string, unknown>) {
                    return headers;
                }
            }

            server = new InversifyExpressServer(container);
            void supertest(server.build()).get("/").set("TestHead", "foo").expect(200, "foo", done);
        });
        ...
        it("should bind a method parameter to a principal with null (empty) details when no AuthProvider is set.", (done) => {
            ...
            server = new InversifyExpressServer(container);
            void supertest(server.build()).get("/").expect(200, "", done);
        });
        ...
        it("should bind a method parameter to a principal with valid details when an AuthProvider is set.", (done) => {
            ...
            void supertest(server.build()).get("/").expect(200, "something", done);
        });
    });
});
